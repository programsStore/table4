...
let masterKey = "$2b$10$JYn6LvXpI.LJFx1P0RkzeeJ5MdcxItZp1klXtnQEmCaY193VRPV7W";
...
async function checkForBinId(data, year, month) {
    ...
    if(response?.metadata?.id) {
        ...
        const putResponse = await setNewData(newData, '64b551a59d312622a380d52d');
...
async function setStorageUrl() {
    ...
    const response = await fetch(url + '/64b551a59d312622a380d52d/latest', {
        method: 'GET',
        headers: {
            'Content-type': 'application/json',
            'X-Master-Key': masterKey,
